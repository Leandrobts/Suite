...
export async function testCSPBypassS1() {
    ...
    try {
        const payloadJS = `try { console.log("[Payload Data:] Alerta data: URI executado!"); alert('XSS S1 via Data URI!'); } catch(e) { console.log("[Payload Data:] Alerta data: URI bloqueado: " + e.message); }`;
        const encodedPayload = btoa(payloadJS);
        const scriptTag = document.createElement('script');
        scriptTag.src = 'data:text/javascript;base64,' + encodedPayload;
        scriptTag.onload = () => logS1("Payload data: URI loaded (check alert/console).", "vuln", "XSS Payload");
    ...
    try {
        ...
        imgTag.src = imgSrc;
        const onerrorPayload = ` try { const target = document.getElementById('xss-target-div'); if (target) { target.innerHTML += '<br><span class="log-vuln">XSS S1 DOM via ONERROR Executado!</span>'; console.log("XSS DOM via onerror OK!"); } else { console.log("Alvo XSS DOM não encontrado."); } alert('XSS_S1_DOM_ONERROR'); } catch(e) { console.log("Erro payload onerror: " + e.message); } `;
    ...
    if (xssTargetDiv) {
        try {
            ...
            link.href = "javascript:try{console.log('[Payload JS Href:] Executado!'); alert('XSS S1 via JS Href!');}catch(e){console.log('[Payload JS Href:] Bloqueado: '+e.message);}";
        ...
        logS1("xss-target-div não encontrado para link JS Href.", "warn", FNAME);
    }
